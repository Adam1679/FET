...
def train_fetel(args, writer, device, gres: exputils.GlobalRes, el_entityvec: ELDirectEntityVec, train_samples_pkl,
    ...
    logging.info(
        'type_embed_dim={} cxt_lstm_hidden_dim={} pmlp_hdim={} nil_rate={} single_type_path={} beta={}'.format (
            type_embed_dim, context_lstm_hidden_dim, pred_mlp_hdim, nil_rate, single_type_path, args.beta))
    ...
    if stack_lstm:
        ...
        model = FETELStack (
            ...
            dropout=dropout, beta=args.beta)
    else:
        model = None
    if device.type == 'cuda':
        model = model.cuda ()
...
def eval_fetel(args, device, gres: exputils.GlobalRes, model, samples: List[ModelSample], entity_vecs, el_probs,
    ...
    for i in range(n_batches):
        ...
        with torch.no_grad():
            ...
            if isinstance (logits, tuple) :
                logits = (args.beta) * logits[0] + (1 - args.beta) * logits[1]
